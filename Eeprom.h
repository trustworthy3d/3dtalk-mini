...
#define EPR_GCODE_LAST_LINE      1024
#define EPR_GCODE_LAST_UNIT      1028
#define EPR_GCODE_LAST_X         1029
#define EPR_GCODE_LAST_Y         1033
#define EPR_GCODE_LAST_Z         1037
#endif

#if ENABLE_RW_SN
#define EPR_SERIAL_NUMBER        1042 // 17byte
#endif
...
public:
	...
	//add by ko
#if (ENABLE_CONTINUE_PRINT || ENABLE_RW_SN)
	static inline void writeByteToEEPROM(uint pos, uint8 value){
		HAL::eprSetByte(pos, value);
	}
	static inline uint8 readByteFromEEPROM(uint pos){
		return HAL::eprGetByte(pos);
	}
#endif
#if ENABLE_CONTINUE_PRINT
	static inline void writeInt32ToEEPROM(uint pos, int32 value){
		HAL::eprSetInt32(pos, value);
	}
	static inline int32 readInt32FromEEPROM(uint pos){
		return HAL::eprGetInt32(pos);
	}
	static inline void writeFloatToEEPROM(uint pos, float value){
		HAL::eprSetFloat(pos, value);
	}
	static inline float readFloatFromEEPROM(uint pos){
		return HAL::eprGetFloat(pos);
	}
#endif
	//add end
