...
/*	// add by tony
	...
	if( heat_box_en == 1 )
		{
		if( tempController[2]->currentTemperatureC > tempController[2]->targetTemperatureC )
           {
			heat_box_fan_pwm = 6;
		   }
		else if( tempController[2]->currentTemperatureC < (tempController[2]->targetTemperatureC-2) )
		   {
			heat_box_fan_pwm = 10;
		   }
	     }
	else if( heat_box_en == 2 )
	    {
	  	if( tempController[2]->currentTemperatureC > 45 )
			heat_box_fan_pwm = 3;
		else{
			heat_box_fan_pwm = 0;
			heat_box_en = 0;
	...
	fan_box_pwm_pos = heat_box_fan_pwm;
#endif

#if PUMP_COOLER_PIN > -1
  	if(( pump_enable == 0 ) && ( tempController[2]->currentTemperatureC < 45 ))
			WRITE(PUMP_COOLER_PIN,0);
