...
void PrintLine::queueCartesianMove(uint8_t check_endstops,uint8_t pathOptimize)
{
    ...
    float axis_diff[4]; // Axis movement in mm 每轴的移动距离(mm)
    int E_dir = 1;	//送丝机方向1pos\-1 neg//modify by derby 8-29
    if(check_endstops)
    ...
    //Find direction
   // Com::printFLN(Com::tDebug,Printer::destinationSteps[E_AXIS]-Printer::currentPositionSteps[E_AXIS]);         //modify by derby 8-29
		...
		//modify by derby
    {
    	...
    	if(p->delta[axis]>=0)
			...
			if(axis == E_AXIS)//add by derby 8-29
			    {	
				E_dir = -1;	//记录转动方向
				//Com::printFLN(Com::tDebug,E_dir);
				}
	...
	if(E_dir<0)	
    {
    	//Com::printFLN(Com::tDebug,"neg- move");
		//Com::printFLN(Com::tDebug,"neg.. no record");
		Printer::last_retract_length = p->delta[E_AXIS];	//保存上一次回抽长度
	}
	else{
		//Com::printFLN(Com::tDebug,Printer::last_retract_length);
		//Com::printFLN(Com::tDebug,p->delta[E_AXIS]);
		if((p->delta[E_AXIS]==Printer::last_retract_length)&&(Printer::last_dir<0))
			Com::printFLN(Com::tDebug,"=..no record");
		else{
    Printer::filamentPrinted += axis_diff[E_AXIS];
			//Com::printFLN(Com::tDebug,"recording");
			}
		 }
	if(p->delta[E_AXIS]!=0)	//不移动e轴时会导致方向判断出错 //add by derby 8-29
		Printer::last_dir = E_dir;
	//add end by derby 8-29
	...
	// stepsRemaining为主轴运动距离，记录主轴剩余运动步数
    p->stepsRemaining = p->delta[p->primaryAxis];
	// p->distance - XYZ空间运动距离与E轴运动距离两者间的最大值
    if(p->isXYZMove())
    {
...
void PrintLine::calculateMove(float axis_diff[],uint8_t pathOptimize)
{
	...
	// 时间间隔有一个最大值，当计算出来的值大于允许最大值时，取允许的最大值
