...
int32_t Printer::last_retract_length = 0;   //add by derby 8-29
int Printer::last_dir = 1;
...
// add by tony
#if defined(PUMP_COOLER_PIN) && PUMP_COOLER_PIN>-1
    SET_OUTPUT(PUMP_COOLER_PIN);
    WRITE(PUMP_COOLER_PIN,LOW);
#endif
#if defined(FILTER_FAN) && FILTER_FAN>-1
    SET_OUTPUT(FILTER_FAN);
    WRITE(FILTER_FAN,LOW);
#endif
// add end
...
#if ENABLE_CONTINUE_PRINT
	Printer::check_last_task();
	Printer::init_power_down_int();
#endif
...
#if ENABLE_CONTINUE_PRINT
void Printer::check_last_task()
{
/*	uint32_t lastLineNumber1 = 20;
	EEPROM::writeInt32ToEEPROM(EPR_GCODE_LAST_LINE, lastLineNumber1);
	Com::print(lastLineNumber1);*/

	uint8_t lastUnit = EEPROM::readByteFromEEPROM(EPR_GCODE_LAST_UNIT);
	if(lastUnit != 255){
		Printer::unitIsInches = lastUnit;
	}
	float x,y,z;
	x = EEPROM::readFloatFromEEPROM(EPR_GCODE_LAST_X);
	y = EEPROM::readFloatFromEEPROM(EPR_GCODE_LAST_Y);
	z = EEPROM::readFloatFromEEPROM(EPR_GCODE_LAST_Z);
	if(!(isnan(x) || isnan(y) || isnan(z))){
		Printer::setOrigin(x, y, z);
	}
	
	uint32_t lastLineNumber = EEPROM::readInt32FromEEPROM(EPR_GCODE_LAST_LINE);
	if(lastLineNumber != 4294967295){
		GCode::external_requestResend(lastLineNumber);
	}

	Com::printFloat(x,5); Com::println();
	Com::printFloat(y,5); Com::println();
	Com::printFloat(y,5); Com::println();
	
	Com::print(lastLineNumber);
}

void Printer::init_power_down_int()
{
/*	以下是准备使用INT7的，但arduino需要什么劳什子映射。如下参考
	http://www.cnblogs.com/Kaysin/archive/2013/02/06/2902197.html
	...
	EIFR |= 0x80;
	SREG |= 0x01;*/
	SET_INPUT(POWER_DOWN_INT_PIN);
	attachInterrupt(4, isr_power_down, LOW);

	//for test
	SET_OUTPUT(POWER_DOWN_TEST_PIN);
}

volatile int led_state = 0;
void Printer::isr_power_down()
{
	uint32_t lastLineNumber = GCode::getLastLineNumber(); //20; //
	EEPROM::writeInt32ToEEPROM(EPR_GCODE_LAST_LINE, lastLineNumber);
	Com::print(lastLineNumber);
	...
	EEPROM::writeByteToEEPROM(EPR_GCODE_LAST_UNIT, Printer::unitIsInches);
	EEPROM::writeFloatToEEPROM(EPR_GCODE_LAST_X, x*(Printer::unitIsInches?0.03937:1));
	EEPROM::writeFloatToEEPROM(EPR_GCODE_LAST_Y, y*(Printer::unitIsInches?0.03937:1));
	EEPROM::writeFloatToEEPROM(EPR_GCODE_LAST_Z, z*(Printer::unitIsInches?0.03937:1));
/*	GCode *code = GCode::peekCurrentCommand();
	code->echoCommand();
	GCode::external_requestResend(65537);*/
	//for test
	led_state = !led_state;
	WRITE(POWER_DOWN_TEST_PIN, led_state);
}
#endif
