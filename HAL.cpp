...
ISR(PWM_TIMER_VECTOR)
{
    ...
    static uint8_t pwm_count_fan3 = 0;
...
//        if((pwm_cooler_pos_set[0] = extruder[0].coolerPWM)>0)
        if( ((pwm_cooler_pos_set[0] = extruder[0].coolerPWM)>0) ||
			((pwm_cooler_pos_set[0] = extruder[0].coolerPWM)>0) )
			WRITE(EXT0_EXTRUDER_COOLER_PIN,1);							// modify by tony
#endif
#if defined(EXT1_HEATER_PIN) && EXT1_HEATER_PIN>-1 && NUM_EXTRUDER>1
...
#if defined(EXT2_HEATER_PIN) && EXT2_HEATER_PIN>-1 && NUM_EXTRUDER>2
#if EXT2_EXTRUDER_COOLER_PIN>-1
		if((pwm_cooler_pos_set[2] = fan_box_pwm_pos)>0) WRITE(EXT2_EXTRUDER_COOLER_PIN,0);
#endif
#endif
    }
    // add end
...
#if EXT2_EXTRUDER_COOLER_PIN>-1
    if(pwm_cooler_pos_set[2] == pwm_count_fan3 && pwm_cooler_pos_set[2]<10) WRITE(EXT2_EXTRUDER_COOLER_PIN,1);
#endif
#endif
#if defined(EXT3_HEATER_PIN) && EXT3_HEATER_PIN>-1 && NUM_EXTRUDER>3
