...
void GCode::external_requestResend(uint32_t lastLineNum)
{
	lastLineNumber = lastLineNum;
	actLineNumber = lastLineNumber - 1;
    requestResend();
}
#endif
    ...
    while(HAL::serialByteAvailable() && commandsReceivingWritePosition < MAX_CMD_SIZE)    // consume data until no data or buffer full
    {
        ...
        else     // Ascii command
        {
            ...
            else
            {
                ...
                if(commentDetected){
					//add by ko, for read or write serial number
					#if ENABLE_RW_SN
					char *str;
					if(0 != (str=strchr((char*)commandReceiving, 'V'))){
						if('0' == str[1]){
							//Com::printF(PSTR("read>"));Com::print(str);Com::println();
							char sn[SN_LENGTH] = "";
							for(int j=0;j<SN_LENGTH;j++){
								sn[j] = EEPROM::readByteFromEEPROM(EPR_SERIAL_NUMBER+j);
							}
							Com::printVFLN((const char*)sn);
						}else if('1' == str[1] && commandsReceivingWritePosition >= (3+SN_LENGTH)){
							//Com::printF(PSTR("write>"));Com::print(&str[3]);Com::println();
							for(int j=0;j<SN_LENGTH;j++){
								EEPROM::writeByteToEEPROM(EPR_SERIAL_NUMBER+j, str[3+j]);
							}
							//must override checksum
						    uint8_t newcheck=0;
						    for(unsigned int i=0; i<2048; i++){
						        if(i==EEPROM_OFFSET+EPR_INTEGRITY_BYTE) continue;
						        newcheck += HAL::eprGetByte(i);
						    }
						    if(newcheck != HAL::eprGetByte(EPR_INTEGRITY_BYTE))
						        HAL::eprSetByte(EPR_INTEGRITY_BYTE, newcheck);
							Com::printVFLN("ok");
						}
						for(int j=0;j<commandsReceivingWritePosition;j++){
							commandReceiving[j] = '\0';
						}
						commandsReceivingWritePosition = 0;
						commentDetected = false;
						return;
						//Com::print((char*)commandReceiving);Com::println();
					}
					#endif
					//add end
					...
					if(i >= 4){
						i = 0;
						if(0 == strcmp(";End", tempBuffer)){
							EEPROM::writeInt32ToEEPROM(EPR_GCODE_LAST_LINE, -1);
						}
					}
					tempBuffer[i++] = ch;
					#endif
